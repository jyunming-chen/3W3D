...
//static t3DModel g_3DModel;				// This holds the 3D Model info that we load in

//static int   g_ViewMode	  = GL_TRIANGLES;	// We want the default drawing mode to be normal
//static GLboolean gLoop = GL_TRUE; // by default, always loop the animation
//static GLboolean gStayEnd = GL_FALSE; // AND, not stay at endframe
...
void MD2MLoad (t3DModel *md2m, char *md2file, char* texfile)
{
	...
	g_LoadMd2.ImportMD2(md2m, md2file, texfile);		
	...
	for(int i = 0; i < md2m->numOfMaterials; i++)
	{
		...
		if(strlen(md2m->pMaterials[i].strFile) > 0)
		{
			...
			CreateTexture(g_Texture, md2m->pMaterials[i].strFile, i);			
		}
...
//
// this function remain intact
//
// should be separate for each model ...
...
// DWORD: unsigned long;
	...
	if (elapsedTime >= (1000.0f / pModel->kAnimationSpeed) )
	{
